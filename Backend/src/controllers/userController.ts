...
export const getUserDetailsById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const [user] = await db.select().from(usersDetails).where(eq(usersDetails.id, Number(id)));
...
export const updateUserDetails = async (req: Request, res: Response) => {
  try {
    ...
    const updatedUser = await db
      ...
      .where(eq(usersDetails.id, Number(id)));
...
export const deleteUserDetails = async (req: Request, res: Response) => {
  try {
    ...
    const deletedUser = await db.delete(usersDetails).where(eq(usersDetails.id, Number(id)));
