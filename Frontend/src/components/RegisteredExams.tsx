...
import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { Download, Calendar, Clock, MapPin, CheckCircle, AlertTriangle, Search, Filter, Loader2 } from 'lucide-react'; // Added Loader2

const API_BASE_URL = 'http://localhost:5000/api'; // Your backend API base URL
const SERVER_URL = 'http://localhost:5000'; // Your server URL for constructing full PDF paths
...
interface RegisteredExamBackend {
  ...
  status: 'approved' | 'pending' | 'rejected'; // More specific status
  applied_at: string;
  hall_ticket_url: string | null;
  exam_time: string | null;
  exam_date: string | null; // Expecting formatted date string like MM/DD/YYYY from backend
  exam_name: string;
  exam_duration: string;
  exam_fee: number | string;
}
...
interface RegisteredExamFrontend {
  ...
  appliedAt: string; // Raw timestamp, will format in JSX
  hallTicketUrl: string | null; // This will be the relative path like /hall_tickets/file.pdf
}
...
const RegisteredExams = () => {
  ...
  const user = JSON.parse(localStorage.getItem('user') || '{}');
  const authId = user.id; // Assuming user object from localStorage has 'id' which is authId
  ...
  useEffect(() => {
    const fetchRegisteredExams = async () => {
      ...
      if (!authId) {
        setError("User authentication ID not found. Please log in.");
        setLoading(false);
        return;
      }

      try {
        const url = `${API_BASE_URL}/examRegistration/getUserExamStats/${authId}`;
        const response = await fetch(url);
        ...
        const mappedData: RegisteredExamFrontend[] = data.map(exam => ({
          ...
          examId: exam.exam_id,
          examName: exam.exam_name,
          date: exam.exam_date || 'N/A', // Already formatted by backend
          time: exam.exam_time || exam.selected_exam_time || 'N/A',
          duration: exam.exam_duration || 'N/A',
          location: exam.assigned_location || (exam.exam_mode === 'online' ? 'Online' : 'N/A'),
          seatNumber: exam.seat_number,
          status: exam.status,
          appliedAt: exam.applied_at,
          hallTicketUrl: exam.hall_ticket_url, // This is the key change - use the real URL
        }));
        setRegisteredExams(mappedData);
      } catch (err: any) {
        console.error("Error fetching registered exams:", err);
        setError(`Failed to load your registered exams: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredExams = registeredExams.filter(exam => {
    ...
    const matchesSearch =
      ...
      exam.applicationId.toLowerCase().includes(searchLower) ||
  ...
  const handleDownloadHallTicket = (exam: RegisteredExamFrontend) => {
    if (exam.hallTicketUrl) {
      // Construct the full URL to the PDF file on the server
      const fullUrl = `${SERVER_URL}${exam.hallTicketUrl}`;
      window.open(fullUrl, '_blank'); // Open PDF in a new tab
    } else {
      alert(`Hall ticket is not yet available for ${exam.examName}.`);
    }
  };

 const getStatusBadge = (status: RegisteredExamFrontend['status']) => {
    switch (status) {
      case 'approved':
        return (
          <div className="flex items-center text-xs text-green-700 bg-green-100 px-2.5 py-1 rounded-full font-medium">
            <CheckCircle className="h-4 w-4 mr-1.5" /> Approved
          </div>
        );
      case 'rejected':
        return (
          <div className="flex items-center text-xs text-red-700 bg-red-100 px-2.5 py-1 rounded-full font-medium">
            <AlertTriangle className="h-4 w-4 mr-1.5" /> Rejected
          </div>
        );
      default: // pending
        return (
          <div className="flex items-center text-xs text-amber-700 bg-amber-100 px-2.5 py-1 rounded-full font-medium">
  ...
  if (error) {
    return (
      <div className="max-w-4xl mx-auto mt-10 p-6 bg-red-50 border-l-4 border-red-500 text-red-700">
        <div className="flex">
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-8">My Registered Exams</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-shadow"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          <div className="relative">
            ...
            <select
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 appearance-none bg-white transition-shadow"
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
            >
              <option value="">All Statuses</option>
      ...
      {filteredExams.length > 0 ? (
        <div className="space-y-6">
          {filteredExams.map(exam => (
            <div key={exam.id} className="bg-white rounded-xl shadow-lg overflow-hidden transition-all hover:shadow-2xl">
              <div className="p-6 border-b border-gray-200">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
                  <div className="mb-3 sm:mb-0">
                    <h2 className="text-xl font-semibold text-gray-800">{exam.examName}</h2>
                    <p className="text-sm text-gray-500 mt-1">Application ID: {exam.applicationId}</p>
                  </div>
                  {getStatusBadge(exam.status)}
                </div>
              </div>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 text-sm mb-6">
                  <div className="flex items-start">
                    <Calendar className="h-5 w-5 text-indigo-500 mr-2.5 shrink-0 mt-0.5" />
                    <div>
                      <p className="font-medium text-gray-700">Date & Time</p>
                      <p className="text-gray-600">{exam.date}</p>
                      <p className="text-gray-600">{exam.time} {exam.duration && `(${exam.duration})`}</p>
                    </div>
                  </div>
                  <div className="flex items-start">
                    <MapPin className="h-5 w-5 text-indigo-500 mr-2.5 shrink-0 mt-0.5" />
                    <div>
                      <p className="font-medium text-gray-700">Location</p>
                      <p className="text-gray-600">{exam.location}</p>
                      {exam.status === 'approved' && exam.seatNumber && (
                        <p className="text-gray-600">Seat: {exam.seatNumber}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-start">
                    <Clock className="h-5 w-5 text-indigo-500 mr-2.5 shrink-0 mt-0.5" />
                    <div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center pt-4 border-t border-gray-200">
                  <Link
                    to={`/status/${exam.applicationId}`} // Make sure this route exists
                    className="text-sm font-medium text-indigo-600 hover:text-indigo-800 transition-colors mb-3 sm:mb-0"
                  >
                    View Application Details
                  </Link>

                  {exam.status === 'approved' && exam.hallTicketUrl ? (
                    <button
                      onClick={() => handleDownloadHallTicket(exam)}
                      className="w-full sm:w-auto flex items-center justify-center px-4 py-2.5 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition-all"
                    >
                      <Download className="h-5 w-5 mr-2" />
                      Download Hall Ticket
                    </button>
                  ) : exam.status === 'pending' ? (
                    <div className="w-full sm:w-auto text-sm font-medium text-amber-600 flex items-center justify-center sm:justify-start px-4 py-2.5 bg-amber-50 rounded-lg">
                      <Clock className="h-5 w-5 mr-2" />
                      Awaiting Approval
                    </div>
                  ) : exam.status === 'rejected' ? (
                     <div className="w-full sm:w-auto text-sm font-medium text-red-600 flex items-center justify-center sm:justify-start px-4 py-2.5 bg-red-50 rounded-lg">
        ...
        <div className="text-center py-12">
          ...
          <h2 className="text-xl font-semibold text-gray-800 mb-2">No Registered Exams Yet</h2>
          <p className="text-gray-600 mb-6">It looks like you haven't registered for any exams, or no exams match your current filters.</p>
          <Link
            to="/register-exam" // Update if your exam registration route is different
            className="inline-flex items-center px-6 py-2.5 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors"
          >
            Explore & Register for Exams
          </Link>
        </div>
      )}
    </div>
